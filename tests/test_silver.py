...
def test_silver_transformation_logic(spark):
    ...
    schema = StructType([
        ...
        StructField("Extra_Col", StringType(), True) # Should be dropped or kept? Logic keeps renamed original cols
    ])
    ...
    data = [
        ...
        ("8002", "2023-10-01", "10:00", "-5", "z"),    # Negative passengers (should be removed)
        ("8003", "2023-10-01", "11:00", "abc", "w"),   # Non-numeric passengers (should be removed)
...
def test_silver_logic_clean(spark):
    ...
    with patch.object(SparkSession, 'table', return_value=df_input) as mock_read:
        ...
        with patch('pyspark.sql.DataFrameWriter.saveAsTable', side_effect=mock_save_as_table, autospec=True):
